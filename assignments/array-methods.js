...
runners.forEach(function(full){
  fullName.push(full.first_name, full.last_name);
})
...
let ticketPriceTotal = runners.reduce((cb, total) => cb + total.donation, 0);
...
runners.forEach(function(entry){
  data.push(entry.first_name, entry.last_name, entry.email);
})
...
let allLower = runners.map(runners => runners.first_name.toLowerCase());
